  ...
  $("#link-read-more-contacts-sent").bind("click", function(){
    ...
    $.get(`/contact/read-more-contacts-sent?skipNumber=${skipNumber}`, function( newContactUsers){
      if (!newContactUsers.length) {
        alertify.notify("Bạn không còn danh sách nào để xem nữa cả.", "error", 5);
          ...
          .append(
            `<li class="_contactList" data-uid="${user._id}">
                <div class="contactPanel">
                    ...
                    <div class="user-remove-request-contact-sent action-danger diplay-important" data-uid="${user._id}">
                        Hủy yêu cầu
                    </div>
                </div>
            </li>`); 
